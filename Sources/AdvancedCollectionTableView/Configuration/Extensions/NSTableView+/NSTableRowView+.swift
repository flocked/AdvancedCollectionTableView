    ...
    func updateContentConfiguration() {
        if let tableView = self.tableView {
          //  Swift.print("isGroupRow", self.row ?? 0, tableView.delegate?.tableView?(tableView, isGroupRow: self.row ?? 0) ?? false)
            Swift.print("automaticRow", (self.contentConfiguration as? NSListContentConfiguration) != nil, (self.contentConfiguration as? NSListContentConfiguration)?.type?.rawValue ?? "nil")
        }
