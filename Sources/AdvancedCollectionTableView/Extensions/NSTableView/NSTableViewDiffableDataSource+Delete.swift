...
public extension NSTableViewDiffableDataSource {
    ...
    var isDeletable: Bool {
        get { getAssociatedValue(key: "NSTableViewDiffableDataSource_isDeletable", object: self, initialValue: false) }
        set {
            guard newValue != isDeletable else { return }
            set(associatedValue: newValue, key: "NSTableViewDiffableDataSource_isDeletable", object: self)
        ...
        if self.isDeletable {
            if keyDownMonitor == nil {
                keyDownMonitor =  NSEvent.addLocalMonitorForEvents(matching: .keyDown, handler: { [weak self] event in
                    ...
                    if isDeletable, let tableView =  (NSApp.keyWindow?.firstResponder as? NSTableView), tableView.dataSource === self {
